...
FROM country_club.Members a
LEFT JOIN country_club.Members b ON a.memid < b.memid
...
FROM (
		...
		SELECT bookings.memid AS memberid,
	   		   facilities.name AS facility_name,
...
JOIN country_club.Members members ON members.memid = sub.memberid

WHERE sub.start_time LIKE '2012-09-14%' AND sub.cost > 30
ORDER BY 3 DESC
...
FROM (
		SELECT facilities.name AS facility_name,
	   		   CASE WHEN bookings.memid = 0 THEN facilities.guestcost * bookings.slots ELSE facilities.membercost * bookings.slots END AS revenue
		FROM country_club.Bookings bookings
		JOIN country_club.Facilities facilities ON bookings.facid = facilities.facid
	 ) sub
GROUP BY 1
